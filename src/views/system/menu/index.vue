...
import MenuAPI, {type MenuForm, type MenuQuery, type MenuVO} from "@/api/menu";
import {MenuTypeEnum} from "@/enums/MenuTypeEnum";

const queryFormRef = ref(ElForm);
const menuFormRef = ref(ElForm);
const queryParams = reactive<MenuQuery>({});// 查询参数
const loading = ref(false);
const menuTableData = ref<MenuVO[]>([]);// 菜单表格数据
const selectedMenuId = ref<number | undefined>();// 选择表格的行菜单ID
const menuOptions = ref<OptionType[]>([]);// 顶级菜单下拉选项
...
const formData = ref({ ...initialMenuFormData.value });// 菜单表单数据
